...
ctrl.controller('timesheet',['$scope','$timeout','project','$routeParams','$location',
    function ($scope,$timeout,project,$routeParams,$location){
        ...
        $scope.editTask = function(pId, tId, notes, adhoc, cId, taskTimeId){
            ...
            if(adhoc == 'ad hoc'){ $location.path('/add_a/'+cId+'/'+tId+'/'+pId+'/'+taskTimeId+'/'+time); }
...
ctrl.controller('expenses',['$scope','$routeParams', 'project', '$location', '$timeout', '$route', '$modal',
    function ($scope, $routeParams, project, $location, $timeout, $route, $modal){
        ...
        $scope.save = function(){
            ...
            var notes = project.getNote();
            if($routeParams.expId){ project.updateExpense($routeParams.d +'/'+ $routeParams.m +'/'+ $routeParams.y,$routeParams.expId,project.getAmount(),project.getNote(),project.expense[$routeParams.d +'/'+ $routeParams.m +'/'+ $routeParams.y][$routeParams.expId]['picture']); }
            else{ project.save(type,$routeParams.item,$routeParams.taskId,notes); }
        }
        $scope.today = function() {
...
ctrl.controller('add_a',['$scope','$routeParams', 'project', '$location', '$timeout',
    function ($scope, $routeParams, project, $location, $timeout){
        ...
        $scope.selectTask = function(id){ if(!$routeParams.taskTimeId){ $location.path('/lists_a/'+id); } }
        $scope.dateOptions = { 'year-format': "'yy'", 'starting-day': 1 };
        /*timepicker*/
        $scope.hstep = 1;
        $scope.mstep = 1;
